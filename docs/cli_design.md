...
- `git-maintenance repo audit`
- `git-maintenance repo rename`
- `git-maintenance repo update-remote`
- `git-maintenance repo convert-protocol`
...
| --- | --- | --- | --- |
| CSV audit (`--audit`) | `git-maintenance repo audit` | `--root` (repeatable; defaults to `.`), `--debug`, `--format csv` (defaults to CSV; future extensibility), `--output` (optional file). Command is read-only. | Use `exec.CommandContext` to run `gh repo view` and `gh api repos/{owner}/{repo}` exactly once per repo. Results cached per repo to minimize API calls. |
| Directory rename (`--rename`) | `git-maintenance repo rename` | `--root`, `--dry-run`, `--require-clean`, `--yes`, `--debug`. | Same metadata resolution as audit. No extra `gh` commands beyond canonical lookup. |
| Update remote to canonical (`--update-remote`) | `git-maintenance repo update-remote` | `--root`, `--dry-run`, `--yes`, `--debug`. | Requires canonical owner/repo from `gh api repos/{owner}/{repo}`. |
| Protocol conversion (`--protocol-from`, `--protocol-to`) | `git-maintenance repo convert-protocol` | `--root`, `--from {https\|git\|ssh}`, `--to {https\|git\|ssh}`, `--dry-run`, `--yes`, `--debug`. | Canonical resolution via `gh api`; no other `gh` usage. |
...
- Roots accept multiple entries; when omitted, default root is the current working directory.
